...
@interface AppViewController1 ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UISearchBarDelegate>
{
...
@implementation AppViewController1
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    // 获取 JSON 文件的 URL
    NSString *jsonURLString = @"http://www.iggcheat.com/eleme.json";
        ...
        NSURLSessionDataTask *dataTask = [session dataTaskWithURL:jsonURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (error) {
                ...
                if (jsonError) {
                    ...
                    // 解析成功，获取参数值
                    self->eleme01 = jsonDict[@"eleme01"];
                    self->eleme02 = jsonDict[@"eleme02"];
                }
            }
        }];
    ...
    -(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        if(indexPath.section == 0){
            if(indexPath.row == 0){
                ...
                cell.textLabel.text = [NSString stringWithFormat:@"🧧饿了么聚合"];
                
                cell.detailTextLabel.text = [NSString stringWithFormat:@"饿了么"];
            }
            
            if(indexPath.row == 1){
                NSString *str = [NSString stringWithFormat:@"5—20无门槛红包\t可叠加使用"];
        ...
        if(indexPath.section == 1){
            if(indexPath.row == 0){
                ...
                cell.textLabel.text = [NSString stringWithFormat:@"🧧饿了么大额"];
                
                cell.detailTextLabel.text = [NSString stringWithFormat:@"饿了么"];
            }
            
            if(indexPath.row == 1){
                NSString *str = [NSString stringWithFormat:@"累计66元卷包\t可叠加使用"];
...
- (void)buttonClicked:(UIButton *)sender {
    ...
    // 根据tag值进行相应的处理
    if (tag == 101) {
        NSURL *URL = [NSURL URLWithString:eleme01];
        [[UIApplication sharedApplication] openURL:URL options:@{} completionHandler:nil];
    } else if (tag == 102) {
        NSURL *URL = [NSURL URLWithString:eleme02];
        [[UIApplication sharedApplication] openURL:URL options:@{} completionHandler:nil];
    }
}
