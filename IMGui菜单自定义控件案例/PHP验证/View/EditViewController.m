...
@interface EditViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UISearchBarDelegate>
{
...
- (void)btnDidClicked{
    ...
    NSString *txturl = [NSString stringWithContentsOfURL:[NSURL URLWithString:[self getUTF8EncodeStringWithURLString:[NSString stringWithFormat:@"%@APPbianji.php?id=2&appid=%@&appname=%@&gg=%@&ggnr=%@&xt=%@&xtjg=%@&lx=%@&userid=%@&pass=%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"后台地址"],[MyInfoJianghu MyUserInfo].appid,self->appname,self->gg,self->ggnr,self->xt,self->xtjg,self->lx,[[NSUserDefaults standardUserDefaults] objectForKey:@"admin账号"],[[NSUserDefaults standardUserDefaults] objectForKey:@"admin密码"]]]] encoding:NSUTF8StringEncoding error:nil];
        //NSLog(@"----->%@",[NSString stringWithFormat:@"%@APPbianji.php?id=2&appid=%@&appname=%@&gg=%@&ggnr=%@&xt=%@&xtjg=%@&lx=%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"后台地址"],[MyInfoJianghu MyUserInfo].appid,self->appname,self->gg,self->ggnr,self->xt,self->xtjg,self->lx]);
        if(txturl){
...
- (void)shuaxin{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [_activityIndicator removeFromSuperview];
        self->appname = self->MYka[@"appname"];
        self->xt = self->MYka[@"心跳"];
        self->xtjg = self->MYka[@"心跳时间"];
        self->gg = self->MYka[@"远程公告"];
        NSString *sm= [self->MYka[@"远程公告内容"] stringByReplacingOccurrencesOfString:@"\n" withString:@"\\n"];
        self->ggnr = sm;
        self->lx = self->MYka[@"类型"];
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{
    ...
    -(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        if(indexPath.section == 0){
            ...
            cell.textLabel.text = @"APP名称";
            cell.detailTextLabel.text = [NSString stringWithFormat:@"%@",self->MYka[@"appname"]];
        ...
        if(indexPath.section == 1){
            ...
            cell.textLabel.text = @"远程公告";
            NSArray *array = [NSArray arrayWithObjects:@"开启",@"关闭", nil];
            UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:array];
            segment.frame = CGRectMake(0,5,150,30);
            if([self->MYka[@"远程公告"] intValue] == 1){
            segment.selectedSegmentIndex = 0;
            }else{
            segment.selectedSegmentIndex = 1;
            }
            segment.apportionsSegmentWidthsByContent = YES;
            [segment addTarget:self action:@selector(gg:) forControlEvents:UIControlEventValueChanged];
        ...
        if(indexPath.section == 3){
            cell.textLabel.text = @"心跳";
            NSArray *array = [NSArray arrayWithObjects:@"开启",@"关闭", nil];
            UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:array];
            segment.frame = CGRectMake(0,5,150,30);
            if([self->MYka[@"心跳"] intValue] == 1){
            segment.selectedSegmentIndex = 0;
            }else{
            segment.selectedSegmentIndex = 1;
            }
            segment.apportionsSegmentWidthsByContent = YES;
            [segment addTarget:self action:@selector(xt:) forControlEvents:UIControlEventValueChanged];
            cell.accessoryView = segment;
        ...
        if(indexPath.section == 4){
            cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
            cell.textLabel.text = @"心跳时间";
            cell.detailTextLabel.text = [NSString stringWithFormat:@"%@",self->MYka[@"心跳时间"]];
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        }
        
        if(indexPath.section == 5){
            cell.textLabel.text = @"APP模式";
            NSArray *array = [NSArray arrayWithObjects:@"收费模式",@"免费模式", nil];
            UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:array];
            segment.frame = CGRectMake(0,5,150,30);
            if([self->MYka[@"类型"] intValue] == 1){
            segment.selectedSegmentIndex = 0;
            }else{
            segment.selectedSegmentIndex = 1;
            }
            segment.apportionsSegmentWidthsByContent = YES;
            [segment addTarget:self action:@selector(lx:) forControlEvents:UIControlEventValueChanged];
            cell.accessoryView = segment;
...
- (void)gg:(UISegmentedControl *)sender{
...
- (void)xt:(UISegmentedControl *)sender{
    if (sender.selectedSegmentIndex == 0) {
        xt = @"1";
        self->MYka[@"心跳"] = @"1";
        usleep(500000);
...
- (void)lx:(UISegmentedControl *)sender{
    if (sender.selectedSegmentIndex == 0) {
        lx = @"1";
        self->MYka[@"类型"] = @"1";
        usleep(500000);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section == 0){
    UIAlertController *KaMi = [UIAlertController alertControllerWithTitle:@"信息" message:@"请输入名称" preferredStyle:UIAlertControllerStyleAlert];
                 ...
                 [KaMi addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                     ...
                     if(tf1.text.length != 0){
                         self->MYka[@"appname"] = tf1.text;
                         self->appname = tf1.text;
                         [self->personalTableView reloadData];
                     }
                 }]];
                 ...
                 [KaMi addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
                     ...
                     textField.placeholder = @"在此输入APP名称";
                     textField.text = self->MYka[@"appname"];
                 }];
    ...
    if(indexPath.section == 2){
    UIAlertController *KaMi = [UIAlertController alertControllerWithTitle:@"信息" message:@"请输入公告内容\n换行\\n" preferredStyle:UIAlertControllerStyleAlert];
                 ...
                 [KaMi addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
                     ...
                     textField.placeholder = @"在此输入公告内容";
                     NSString *sm= [self->MYka[@"远程公告内容"] stringByReplacingOccurrencesOfString:@"\n" withString:@"\\n"];
                     textField.text = sm;
                 }];
    ...
    if(indexPath.section == 4){
    UIAlertController *KaMi = [UIAlertController alertControllerWithTitle:@"信息" message:@"请输入心跳叫间隔/秒" preferredStyle:UIAlertControllerStyleAlert];
                 ...
                 [KaMi addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
                     ...
                     textField.placeholder = @"在此输入心跳叫间隔";
                     textField.text = self->MYka[@"心跳时间"];
                 }];
