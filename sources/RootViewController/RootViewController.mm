...
@implementation RootViewController {
    ...
    SHRootCtrl *_localSHRootCtrl;  // Only for debugging
...
#else
    if (enabled && _localSHRootCtrl == nil) {
        _localSHRootCtrl = [[SHRootCtrl alloc] init];
        [self presentViewController:_localSHRootCtrl animated:YES completion:nil];
    } else {
        [_localSHRootCtrl dismissViewControllerAnimated:YES completion:nil];
        _localSHRootCtrl = nil;
    }
#endif
}
...
- (void)tapAuthorLabel:(UITapGestureRecognizer *)sender
{
    ...
    NSString *repoURLString = @"https://SystemHelper.app";
