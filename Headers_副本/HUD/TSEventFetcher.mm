...
static void __TSEventFetcherCallback(void *info) {
    ...
    for (UITouch *aTouch in myAry) {
        switch (aTouch.phase) {
            ...
            case UITouchPhaseCancelled: toRemove = aTouch; break;
            case UITouchPhaseBegan: toStationarify = aTouch; break;
            default: break;
        }
...
+ (NSInteger)receiveAXEventID:(NSInteger)eventId atGlobalCoordinate:(CGPoint)coordinate withTouchPhase:(UITouchPhase)phase inWindow:(UIWindow *)window onView:(UIView *)view {
    BOOL deleted = NO;
    UITouch *touch = nil;
    BOOL needsCopy = NO;
    if (toRemove != nil) {
    ...
    if (toStationarify != nil) {
        ...
        toStationarify = nil;
        if (touch.phase == UITouchPhaseBegan) [touch setPhaseAndUpdateTimestamp:UITouchPhaseStationary];
    }
    eventId -= 1;
    ...
    if (newState) {
        if (phase == UITouchPhaseEnded || phase == UITouchPhaseCancelled) return deleted;
        ...
        if (touch.phase == UITouchPhaseBegan && phase == UITouchPhaseMoved) return deleted;
